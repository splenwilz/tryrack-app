...
async function readImageAsBase64(imageUri: string): Promise<{ data: string; mimeType: string }> {
    ...
    // If already a remote URL, return as-is (we'll handle differently)
    if (imageUri.startsWith('http://') || imageUri.startsWith('https://')) {
        // For remote URLs, we'd need to fetch and convert, but for now assume local
        throw new ApiError(400, 'Remote image URLs not yet supported. Please use local file URI.');
    }
